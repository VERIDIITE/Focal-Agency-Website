...
export default function ClientWinsSection({ onOpenEstimator }) {
  ...
  return (
    <section id="clients" className="relative py-28 px-4 sm:px-8 bg-[#07090e] text-white border-y border-white/10 transition-colors overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto space-y-12 relative z-10">
        ...
        {/* Client Logo Grid */}
        <ScrollReveal animation="scale-up" stagger={true} className="grid grid-cols-2 md:grid-cols-4 border-t border-l border-white/10 bg-white/5 backdrop-blur-md">
          {clients.map((client, idx) => (
            <div
              key={idx}
              className="p-10 sm:p-14 border-r border-b border-white/10 flex flex-col items-center justify-center text-center space-y-3 hover:bg-white/10 transition-colors group cursor-pointer"
              onClick={onOpenEstimator}
            >
              <span className="font-display font-extrabold text-2xl sm:text-3xl text-white group-hover:scale-105 transition-transform tracking-tight">
                {client.name}
              </span>
              <div className="space-y-1">
                <p className="text-[11px] font-mono uppercase tracking-wider text-slate-400">{client.category}</p>
                <p className="text-xs font-bold text-cyan-400">{client.metric}</p>
              </div>
            </div>
          ))}
        </ScrollReveal>
