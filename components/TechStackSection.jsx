...
export default function TechStackSection({ onOpenEstimator }) {
  ...
  return (
    <section id="tech-stack" className="relative py-12 sm:py-28 bg-[#07090e] text-white border-t border-white/10 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 space-y-16 relative z-10">
        ...
        <ScrollReveal animation="fade-up">
          <div className="p-5 sm:p-10 rounded-2xl sm:rounded-3xl bg-white/5 border border-white/15 backdrop-blur-xl shadow-2xl grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8 divide-y lg:divide-y-0 lg:divide-x divide-white/10">
            {trustMetrics.map((m, idx) => (
              <div key={idx} className={`space-y-1 sm:space-y-2 text-center lg:text-left ${idx > 0 ? 'pt-4 lg:pt-0 lg:pl-8' : ''}`}>
                <div className="flex items-center justify-center lg:justify-start gap-1.5 sm:gap-2">
                  <m.icon className="w-4 h-4 sm:w-5 sm:h-5 text-cyan-400 shrink-0" />
                  <span className="font-display font-extrabold text-2xl sm:text-4xl text-white tracking-tight">
                    {m.value}
                  </span>
                </div>
                <p className="text-[10px] sm:text-xs font-mono font-medium text-slate-400 uppercase tracking-wider">
                  {m.label}
                </p>
              </div>
            ))}
      ...
      <div className="relative mt-8 sm:mt-12 space-y-3 sm:space-y-4 overflow-hidden py-4">
        {/* Left & Right Fade Vignette Masks */}
        <div className="absolute left-0 top-0 bottom-0 w-16 sm:w-48 bg-gradient-to-r from-[#07090e] to-transparent z-20 pointer-events-none" />
        <div className="absolute right-0 top-0 bottom-0 w-16 sm:w-48 bg-gradient-to-l from-[#07090e] to-transparent z-20 pointer-events-none" />
        ...
        <div className="animate-marquee flex items-center gap-4 sm:gap-6 px-4">
          {[...row1Logos, ...row1Logos].map((tech, idx) => (
            <div
              ...
              className="px-4 sm:px-6 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl bg-white/5 border border-white/10 flex items-center gap-2.5 sm:gap-3 shrink-0 backdrop-blur-md hover:border-cyan-400/60 hover:bg-white/10 transition-all cursor-pointer group shadow-lg"
            >
              <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-cyan-400 group-hover:scale-125 transition-transform" />
              <span className="font-display font-bold text-xs sm:text-sm text-white group-hover:text-cyan-300 transition-colors">
                {tech.name}
              </span>
        ...
        <div className="animate-marquee-reverse flex items-center gap-4 sm:gap-6 px-4">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 sm:pt-10 text-center">
        <button
          onClick={() => onOpenEstimator('Custom Technical Architecture')}
          className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-extrabold text-xs uppercase tracking-wider transition-all shadow-xl shadow-cyan-500/20 hover:scale-105"
        >
